...
public class PelGroupController extends BaseController {
    ...
    public void details() {
        ...
        JPelgroupdrill pelgroupDrill = PelGroupDrillQuery.me().getByPelGroup(id);
        JDrillcode drill = new JDrillcode();
        if(pelgroupDrill!=null){
        	drill  =  DrillcodeQuery.me().findById(pelgroupDrill.getDrillId());
        } 
    ...
    public void add() {
        //List<JDrillcode> drillcodes = DrillcodeQuery.me().getAll();
        JDrillcode drill = DrillcodeQuery.me().getActiveDrill();
        
        if(drill == null){
        	//renderAjaxResultForError("暂无进行中的训练，无法添加！！！");
        	//render("pelgroup.html");
        	//return;
        }
        ...
        List<JPel> pelgroups = PelQuery.me().getByCurrentDrill();
        
        //setAttr("drillcodes", drillcodes);
        setAttr("seats", seats);
    ...
    public void addSave() {
        ...
        JDrillcode drill =  DrillcodeQuery.me().getActiveDrill();
        if(drill != null){
        	JPelgroupdrill pelgroupdrill = new JPelgroupdrill();
        	pelgroupdrill.setDrillId(drill.getId());
        	pelgroupdrill.setPelgroupId(jo.getId());
        	pelgroupdrill.saveOrUpdate();
        }
    ...
    public void edit() {
        ...
        List<JPel> pel = PelQuery.me().getByCurrentDrill();
        List<JPel> pelgroupPel = PelQuery.me().getGroupAll(id);
        //获取全部席位
        //List<JSeat> seats = SeatQuery.me().getAll();
        //根据当前训练获取席位
        List<JSeat> seats = SeatQuery.me().getByCurrentDrill();
        ...
        setAttr("seats", seats);
        setAttr("pelgroupPel", pelgroupPel);
        setAttr("pel", pel);
    ...
    public void del() {
        ...
        if (seatQuery.isEmpty()) {
            PelgroupPelQuery.me().delectByPelgroupId(id);//删除图元组与图元的关联
            
            PelGroupDrillQuery.me().deleteByPelgroupId(id, DrillcodeQuery.me().getActiveDrill().getId());//删除图元组与训练的关联
            //JPelgroupdrill pelgroupdrill = new JPelgroupdrill();
            //pelgroupdrill.doDelete("pelgroupId = ? and ", id);
            //PelGroupDrillQuery
