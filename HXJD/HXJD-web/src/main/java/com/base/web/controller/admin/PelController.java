...
import com.base.model.JPeldrill;
 ...
 */
@RouterMapping(url = "/admin/pel", viewPath = "/view/admin/pel")
@RouterNotAllowConvert
public class PelController extends BaseController {
    ...
    public void add() {
        //List<JDrillcode> drillList = DrillcodeQuery.me().getAll();
    	//setAttr("drillList", drillList);
    	render("add.html");
    }
    public void addSave() {
        JPel jo = getModel(JPel.class);       
//        int drillId = getParaToInt("drill");
        JDrillcode drill = DrillcodeQuery.me().getActiveDrill();
    	if(drill != null){
    		jo.setDrill(drill.getId());
    	}
        
        boolean a = jo.saveOrUpdate();
        if(a){        	
