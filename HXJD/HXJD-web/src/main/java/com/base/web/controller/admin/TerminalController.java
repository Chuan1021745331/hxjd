...
public class TerminalController extends BaseController {
    ...
    public void addSave() {
        ...
        String sdNum = jt.getSdnum();
        String terminalNum = jt.getTerminalNum();
        JTerminal ter =  TerminalQuery.me().getTerminalByTernum(terminalNum);
        if(ter!=null){
        	renderAjaxResultForError("该设备已被注册！！！");
            return;
        }
        JTerminal ter_ = TerminalQuery.me().getTerminalBysdnum(sdNum);
        if(ter_!=null){
        	renderAjaxResultForError("该SD卡已被注册！！！");
            return;
        }
        ...
        if (a) {
        	...
        	if(unTerminalList!=null && unTerminalList.size()>0){
            	for(int i= 0; i<unTerminalList.size();i++){
            		Map<String, String> map = unTerminalList.get(i);
            		if(sdNum.equals(map.get("sdMac")) && terminalNum.equals(map.get("devMac"))){
            			unTerminalList.remove(i);
            		}
            	}
            }
