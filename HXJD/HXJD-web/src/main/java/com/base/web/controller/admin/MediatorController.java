...
public class MediatorController extends BaseController {
     ...
     */
    public void add() {
        List<JTerminal> jTerminals = TerminalQuery.me().getAllTerminal();//所有终端
        List<JMediatorterminal> jMediatorTerminals = MediatorTerminalQuery.me().getByCurrentDrill();//当前训练的mediatorTerminal
        
        List<JSeat> jeat = new ArrayList<>();
        List<JSeat> seats = SeatQuery.me().getByCurrentDrill();
        List<JMedseat> medseat = MeaSeatQuery.me().getAll();
        //遍历出已选的席位
        boolean tags = false;
        for (JSeat jSeat: seats){//------------------------------------------------
            for (JMedseat jMedseat: medseat){
            	JMediator med =  MediatorQuery.me().findById(jMedseat.getMediatorId());
             	JDrillcode drill =  DrillcodeQuery.me().getActiveDrill();
            	if(med!= null){
            		if(drill != null){
            			if(med.getDrillId().intValue() != drill.getId().intValue()){//不是当前训练的
                               jeat.add(jSeat);
                               tags = true;
                		} else if (jMedseat.getSeatId()==jSeat.getId()){//当前训练已经绑定的席位
                            jeat.add(jSeat);
                            tags = true;
                        }
            		}           		
        ...
        if(jMediatorTerminals!=null&&jTerminals!=null){
            for(JMediatorterminal jMediatorTerminal:jMediatorTerminals){
                for(JTerminal jTerminal:jTerminals){
                	//jMediatorTerminal.getTerminalId()
        ...
        //群组
        List<JGroup> groups = GroupQuery.me().getAllGroup();//要改为当前训练的组
...
//        System.out.println("/admin/mediator-----2----->addSave=");
        ...
        if(jm!=null){
        	JDrillcode drill =  DrillcodeQuery.me().getActiveDrill();
        	if(drill != null){
        		jm.setDrillId(drill.getId());//所属训练
        	}
        	      	
        	ajm= jm.save();
            JGroupmediator groupmediator=new JGroupmediator();
            if("1".equals(jm.getCamp())){//蓝
    ...
    public void details() {
        ...
        JDrillcode drill = DrillcodeQuery.me().findById(mediator.getDrillId());
    ...
    public void edit() {
        ...
        //获取全部席位
        //List<JSeat> seats = SeatQuery.me().getAll();//所有的
        List<JSeat> seats = SeatQuery.me().getByCurrentDrill();//当前训练所有的
        //获取绑定的席位
        List<JSeat> existSeat = SeatQuery.me().getSeatAll(id);//该调理员的席位
        List<JSeat> jeatt =new ArrayList<>();
        List<JMedseat> medseat = MeaSeatQuery.me().getAll();//已绑定的
        //遍历出已选的席位
/*        boolean tags = false;
        for (JSeat jSeat: seats){
            for (JMedseat jMedseat: medseat){
                if (jMedseat.getSeatId()==jSeat.getId()){
                    if (!existSeat.isEmpty()){//不为空
                        							//一个人一个席位
...
//        		if(jSeat.getId()==existSeat.get(0).getId()){
//        			jeatt.add(jSeat);
//        		}
//        	}	        	
//        }
...
//        jt.putCache(jt.getOptionKey(), jo.getOptionValue());
        ...
        if(ajm){
            Integer[] ids=getParaValuesToInt("groupId");     
            JGroupmediator groupmediator=new JGroupmediator();
            groupmediator.doDelete("mediatorId=?",jm.getId());//删掉该调理员对应的所有j_groupmediator记录
            /*if("1".equals(jm.getCamp())){//蓝色
            	groupmediator.setGroupId(2);
            }else {//红色
            	groupmediator.setGroupId(3);
            }*/
            groupmediator.setGroupId(GroupQuery.me().getGroupByCamp(Integer.parseInt(jm.getCamp())).getId());
            groupmediator.setMediatorId(jm.getId());
            groupmediator.save();
    ...
    public void del() {
      ...
      /*  if(ob!=null)
        ...
        OffUserQuery.me().delByMedid(id.toString());
