...
//        drill.setParentId(stageParentId);
        ...
        for(Map.Entry<Integer,List<PushObjectDto>> entry : map.entrySet()){
            ...
            if(terminal!=null){
                ChannelContext context_= (ChannelContext) IMcacheMap.cacheMap.get(terminal.getSdnum());
                SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
                String date=df.format(new Date());
                if(context_!=null){
                    //在线
                	try {
                		List<PushObjectDto> polist_ = entry.getValue();
                		ScoreBean scoreBean_=new ScoreBean();
                		scoreBean_.setMsg(polist_);
                		scoreBean_.setSendTime(date);
                		scoreBean_.setMsgType(OffMsgType.SCORE_TYPE+"");
                		IMPacket packet_=new IMPacket();
                		ResponseDto responseDto_=new ResponseDto();

                		responseDto_.setCode(109);
                		responseDto_.setTime(date);
                    	responseDto_.setResponseStatus(1);
                    	responseDto_.setHandle(2);

                    	responseDto_.setData(scoreBean_);
                    	String respo_= JSON.toJSONString(responseDto_);
//                    String respo= JSON.toJSONString("ma dong de ......");
                    	byte[] doZipMsg = new byte[0];
                   
                        doZipMsg = GZipUtil.doZip(respo_);
                        packet_.setBody(doZipMsg);
                        Aio.send(context_,packet_);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
