  ...
  LarryTab.prototype.init = function() {
    ...
    $('#larry_tab').on("click", "#larry_tab_title li", function() {
    	...
    	if(id==0){
            ...
            var nowTabsrc = nowIframe.attr("src");
            nowTabDataGroup = typeof(nowTabDataGroup) == "undefined"?0:nowTabDataGroup;
            $("#larry_top_menu > li").removeClass("layui-this");
            $("#larry_top_menu>li").each(function(){
            	if($(this).find("a").attr("data-group")==nowTabDataGroup){
            		$(this).click();
            	}
            });
            ...
            $('#larry_left_menu a').each(function(){
	            　　		if($(this).attr("data-url")==nowTabsrc){
  ...
  LarryTab.prototype.tabAdd = function(data) {
	  ...
	  $.ajax({
		  ...
		  success:function(json){
			  if(json.errorCode==1){
				  top.location=bpath+'/admin/login';
				  return;
			  }else{
  ...
  function getHtml(data, topStatus) {
    ...
    if (topStatus == 'on') {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        ulHtml.top += '<i class="icon iconfont ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
          ...
          for (var j = 0; j < data[i].children.length; j++) {
            ...
            if (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0) {
              ...
              if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
                // 暂时只定义一种用法
                ulHtml.left[i] += '<i class="icon iconfont ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '"></i>';
              }
              ...
              for (var k = 0; k < data[i].children[j].children.length; k++) {
                ...
                ulHtml.left[i] += '<a data-url="' + bpath +'/'+ data[i].children[j].children[k].url + '">';
                if (data[i].children[j].children[k].icon !== undefined && data[i].children[j].children[k].icon !== '') {
                  // 暂时只定义一种用法
                  ulHtml.left[i] += '<i class="icon iconfont ' + data[i].children[j].children[k].icon + '" data-icon="' + data[i].children[j].children[k].icon + '"></i>';
                }
              ...
              var dataUrl = (data[i].children[j].url !== undefined && data[i].children[j].url !== '') ? 'data-url="' +bpath +'/'+ data[i].children[j].url + '"' : '';
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
          ...
          if (data[i].icon !== undefined && data[i].icon !== '') {
            ulHtml += '<i class="icon iconfont ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
          }
          ...
          for (var j = 0; j < data[i].children.length; j++) {
            ulHtml += '<dd>';
            ulHtml += '<a data-url="' +bpath +'/'+ data[i].children[j].url + '">';
            if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
              ulHtml += '<i class="icon iconfont ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '"></i>';
            }
          ...
          var dataUrl = (data[i].url !== undefined && data[i].url !== '') ? 'data-url="' +bpath +'/'+ data[i].url + '"' : '';
          ulHtml += '<a ' + dataUrl + '>';
          ulHtml += '<i class="icon iconfont ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
          ulHtml += '<cite>' + data[i].title + '</cite>';
          ulHtml += '</a>';
